...
export const authOptions: NextAuthOptions = {
      ...
      async authorize(credentials, req) {
        try {
          ...
          if (!user.emailVerified) {
            // Send verification email again and throw error
    ...
    GoogleProvider({
      ...
      profile(profile) {
        return {
          ...
          emailVerified: new Date(), // Mark as verified since Google verifies emails
        };
      },
    }),
    GitHubProvider({
  ...
  events: {
    async createUser({ user }) {
      // This is triggered when a new user signs up via OAuth
      // Mark OAuth users as verified by default since they've verified their email with the provider
      try {
